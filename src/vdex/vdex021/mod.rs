...
// Android version: 10.0.0-11.0.0
//
// This version appears to be identical to 019 other than:
//
//  - The changes to the verifier_deps, which now have bit vectors for `verified_classes` and `redefined_classes`
//  - The new `bootclasspath_checksum` and `class_loader_context` sections and their respective size fields in `verifier_deps_header`
//
//
// File format:
//
...
//     - dex_shared_data: [u8; verifier_deps_header.dex_shared_data_size]
//         - This area of the VDex is the result of all contained Dex data sections being concatenated together and deduplicated
//     - verifier_deps: ???
//     - quickening_info: ??? TODO
//     - bootclasspath_checksum: c_str or [u8; verifier_deps_header.bootclasspath_checksum_size] however you want to treat it
//     - class_loader_context: c_str or [u8; verifier_deps_header.class_loader_context_size] however you want to treat it

pub const MAGIC: [u8; 4] = [b'v', b'd', b'e', b'x'];
pub const VERIFIER_DEPS_VERSION: [u8; 4] = [b'0', b'2', b'1', b'\0'];
...
impl<'a, TRead: IOread<Endian> + Seek> IoReader<'a, TRead> {
    ...
    // TODO: This should be updated to optionally take the `dex_files` and just dynamically resize the bit vectors if they're missing...
    //       This would at least let you read the verifier deps from an "empty" VDex. Which appears to be every VDex of this version...
    pub fn read_verifier_deps(
        &mut self,
        verifier_deps_header: &VerifierDepsHeader,
        dex_section_header: &DexSectionHeader,
        dex_files: &Vec<(u32, CDex)>,
        ...
        if verifier_deps_header.verifier_deps_size > 0 {
            ...
            let dex_files_size = dex_section_header.dex_section_size as usize
            ...
            for (_, dex_file) in dex_files {
                ...
                let verified_classes = self
                    .read_verifier_deps_u16_sparse_bit_vector(dex_file.class_defs.len(), false)?;
                let redefined_classes =
                    self.read_verifier_deps_u16_sparse_bit_vector(dex_file.class_defs.len(), true)?;
    ...
    fn read_verifier_deps_u16_sparse_bit_vector(
        &mut self,
        num_class_defs: usize,
        sparse_value: bool,
    ) -> Result<Vec<bool>> {
        let mut result: Vec<bool> = Vec::with_capacity(num_class_defs);
        result.resize(num_class_defs, !sparse_value);

        let num_entries: u32 = leb128::decode_uleb128(self.reader)?;

        for _ in 0..num_entries {
            let index: u32 = leb128::decode_uleb128(self.reader)?;
            let index: u16 = match u16::try_from(index) {
                Ok(value) => value,
                Err(_) => return Err(Error::Malformed(format!(
                    "VDex contained malformed sparse bit vector value `{}`, max value allowable is `{}`",
                    index,
                    u16::MAX,
                )))
            };

            if usize::from(index) >= result.len() {
                return Err(Error::Malformed(format!(
                    "VDex contained malformed sparsed bit vector index `{}`, there are only `{}` class defs",
                    index,
                    num_class_defs,
                )));
            }

            result[usize::from(index)] = sparse_value;
        }
    ...
    pub fn read_bootclasspath_checksum(
        &mut self,
        verifier_deps_header: &VerifierDepsHeader,
        dex_section_header: Option<&DexSectionHeader>,
    ) -> Result<Vec<u8>> {
        if verifier_deps_header.bootclasspath_checksum_size > 0 {
            let verifier_deps_header_size = std::mem::size_of::<VerifierDepsHeader>() as u64;
            let checksums_size = (std::mem::size_of::<u32>()
                * verifier_deps_header.number_of_dex_files as usize)
                as u64;
            ...
            let dex_files_size = if let Some(dex_section_header) = dex_section_header {
                dex_section_header.dex_section_size as u64
                    + dex_section_header.dex_shared_data_size as u64
            } else {
                0
            };
            let verifier_deps_size = u64::from(verifier_deps_header.verifier_deps_size);
            let quickening_info_size = if let Some(dex_section_header) = dex_section_header {
                u64::from(dex_section_header.quickening_info_size)
            } else {
                0
            };
            let section_start_offset = verifier_deps_header_size
            ...
            self.reader.seek(SeekFrom::Start(section_start_offset))?;

            let mut result: Vec<u8> =
                Vec::with_capacity(verifier_deps_header.bootclasspath_checksum_size as usize);
            result.resize(verifier_deps_header.bootclasspath_checksum_size as usize, 0);
    ...
    pub fn read_class_loader_context(
        &mut self,
        verifier_deps_header: &VerifierDepsHeader,
        dex_section_header: Option<&DexSectionHeader>,
    ) -> Result<Vec<u8>> {
        if verifier_deps_header.bootclasspath_checksum_size > 0 {
            ...
            let bootclasspath_checksum_size =
                u64::from(verifier_deps_header.bootclasspath_checksum_size);
            let section_start_offset = verifier_deps_header_size
                + checksums_size
                + dex_section_header_size
                + dex_files_size
                + verifier_deps_size
                + quickening_info_size
                + bootclasspath_checksum_size;

            self.reader.seek(SeekFrom::Start(section_start_offset))?;

            let mut result: Vec<u8> =
                Vec::with_capacity(verifier_deps_header.class_loader_context_size as usize);
            result.resize(verifier_deps_header.class_loader_context_size as usize, 0);
